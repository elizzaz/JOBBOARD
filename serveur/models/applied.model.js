...
Applied.findBycandidate = (credentials, result) => {
  sql.query(
    `SELECT advertisements.title, advertisements.description, advertisements.contrat_type FROM advertisements INNER JOIN applied ON advertisements.id = applied.advertisement_id WHERE applied.candidate_id = ${credentials.candidate_id}
`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
